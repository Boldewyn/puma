...
?>
<h2><?php printf(__('Upload new attachment for publication &ldquo;%s&rdquo;'), 
                 h($publication->title)) ?></h2>
<?php if (getConfigurationSetting('SERVER_NOT_WRITABLE')!= 'TRUE'): ?>
  <div class="editform">
    <?php echo form_open_multipart('attachments/commit') ?>
      ...
      <p>
        <label class="block" for="attachments_add_file_upload"><?php
          _e('Select a file') ?></label>
        <input type="file" class="file" name="upload" id="attachments_add_file_upload" />
      </p>
      <p>
        <label class="block" for="attachments_add_file_name"><?php
          _e('Set new name (blank: keep original name)') ?></label>
...
<div class="editform">
  <?php echo form_open_multipart('attachments/commit');?>
    ...
    <p>
      <label class="block" for="attachments_add_link_location"><?php 
        _e('Location of file or web address') ?></label>
      <input type="text" class="text" name="location" id="attachments_add_link_location" />
    </p>
    <p>
      <label class="block" for="attachments_add_link_name"><?php
        _e('Set internal name (blank: keep original name)') ?></label>
