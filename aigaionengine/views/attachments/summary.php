...
<p>
  <?php if ($attachment->isremote): ?>
    <a href="<?php echo prep_url($attachment->location)?>" rel="external"
       title="<?php printf(__('Visit %s'), h($attachment->name))?>"><?php echo $name ?></a>
  <?php else:
  ...
  if ($userlogin->hasRights('attachment_edit') && 
    ...
    <br/>
    <?php _a('attachments/delete/'.$attachment->att_id, '['.__('delete').']', 
      array('title'=>sprintf(__('Delete %s'), h($attachment->name)))) ?>
    <?php _a('attachments/edit/'.$attachment->att_id, '['.__('edit').']', 
      array('title'=>sprintf(__('Edit information for %s'), h($attachment->name)))) ?>
    <?php if ($attachment->ismain): ?>
        <?php _a('attachments/unsetmain/'.$attachment->att_id, 
          '['.__('unset main').']', array('title'=>__('Unset as main attachment'))) ?>
    <?php else: ?>
        <?php _a('attachments/setmain/'.$attachment->att_id, 
          '['.__('set main').']', array('title'=>__('Set as main attachment'))) ?>
    <?php endif; ?>
